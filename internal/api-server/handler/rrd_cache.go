...
func (c *RrdResourceCache) Init() {

	c.IsStopped = false

	c.Config.NetApp.ProjectMode = "qtree"
	c.Config.NetApp.ProjectRoot = PathProjectRrd
	c.Config.NetApp.VolumeProjectQtrees = "rrd4project"
	...
	go func() {
		for {
			select {
			case <-ticker.C:
				log.Infof("refreshing rrd cache")
				c.refresh()
				log.Infof("rrd cache refreshed")
			case m := <-c.Notifier:
				...
				log.Infof("rrd cache updated for project: %s", p.ProjectID)

			case <-c.Context.Done():
				log.Infof("rrd cache refresh stopped")
				c.IsStopped = true
				return
			}
		}
	...
	log.Infof("rrd cache initalized")
}

// refresh update the cache with up-to-data project resources.
func (c *RrdResourceCache) refresh() {
...
func (c *RrdResourceCache) getResource(pnumber string, force bool) (*projectResource, error) {
	...
	if !ok || force {
		...
		storage, members, err := getRrdResource([]*filer.QuotaReport{}, pnumber, c.Config)
		if err != nil {
...
// getRrdResource retrieves storage resource and access roles of a given project.
func getRrdResource(netappQuotaReports []*filer.QuotaReport, pnumber string, cfg config.Configuration) (*models.StorageResponse, []*models.Member, error) {

	path, err := pid2rrdpath(pnumber)
	...
	if system == "netapp" {
		for _, r := range netappQuotaReports {
			if r.QTree.Name == fmt.Sprintf("rrd%s", pnumber) {
...
// pid2rrdpath converts project id to file system path of project's rrd.
func pid2rrdpath(pid string) (string, error) {
	var path string
	if matched, _ := regexp.MatchString("^[0-9]{7,}", pid); matched {
		// input pid is a project number
		path = filepath.Join(PathProjectRrd, fmt.Sprintf("rrd%s", pid))
	} else {
